...
while (1)
    ...
    while packindex<=numofpacket
        ...
        img0(((result(2)*256+result(1)))*bytesperpacket+1:((result(2)*256+result(1))+1)*bytesperpacket)=result(1:bytesperpacket);
        img0(((result(2)*256+result(1)))*bytesperpacket+(1:2))=img0(((result(2)*256+result(1)))*bytesperpacket+(3:4));
    ...
    if (stop==0)
        C=img0(1:2:end)+img0(2:2:end).*256;
        imgB = bitshift(bitand(C,63488),-8); %十进制 63488 为二进制 11111000 00000000
        imgG = bitshift(bitand(C,2016),-3);  %十进制 2016  为二进制 00000111 11100000   
        imgR = bitshift(bitand(C,31),3);     %十进制 31    为二进制 00000000 00011111     
        im888 = uint8(reshape(cat(3,imgR,imgG,imgB),[IMGHW IMGHW 3])); %组合RGB数据，然后把16位强制转化为8位

        axes(handles.axes1);
        imshow(imrotate(im888,-90))
        %view(90,-90);
        drawnow;
    end
end
