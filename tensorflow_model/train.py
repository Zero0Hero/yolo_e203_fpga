...
if __name__ == "__main__":
    ...
    config.gpu_options.per_process_gpu_memory_fraction = 0.75  # 指定显存分配比例
    ...
    checkpoint = ModelCheckpoint('model_data/yolo0718.h5',
    ...
    if True:
        ...
        if Cosine_scheduler:
            ...
            # 学习率
            reduce_lr = WarmUpCosineDecayScheduler(learning_rate_base=learning_rate_base, total_steps=total_steps,
                                                   warmup_learning_rate=1e-5, warmup_steps=warmup_steps,
                                                   hold_base_rate_steps=num_train // 2, min_learn_rate=1e-7)
            model.compile(optimizer=Adam(), loss={'yolo_loss': lambda y_true, y_pred: y_pred})
        else:
            reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=30, verbose=1)
            model.compile(optimizer=Adam(learning_rate_base), loss={'yolo_loss': lambda y_true, y_pred: y_pred})
